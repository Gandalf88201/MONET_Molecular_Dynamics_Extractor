...
- **Displacement ellipsoids (atoms)**: the mean-square displacement tensor U of each atom (Å², Cartesian axes of the first frame, after the alignment) is drawn on the structure as a translucent ellipsoid enclosing 50, 90 or 99 % of the positions, with an optional magnification, in the colour of the map. The six components U11 … U23 are added to the statistics CSV. These are the MD counterpart of crystallographic anisotropic displacement parameters for internal motion: rigid-body translation and libration are removed, and classical nuclei have no zero-point motion.
- **Thermal ellipsoid files**: *More analyses › Thermal ellipsoids (ADP) → PDB* writes the average positions with `ANISOU` records (Cartesian axes of the first frame, B = 8π²U<sub>eq</sub>); *… in crystal axes → CIF* writes fractional positions in the first-frame cell with `_atom_site_aniso_U_ij` in the IUCr convention (U<sub>cart</sub> = A N U N Aᵀ, space group P1). Both open as thermal ellipsoids in Mercury, VESTA, Olex2, PyMOL or ORTEP. They are a plugin (`plugins/displacement_ellipsoids.py`) using the same calculation.
...
A shared **Electronic states** row sets charge and spin multiplicities (one file per multiplicity: 1 → `sing`, 2 → `doub`, 3 → `trip`, 4 → `quar`, 5 → `quin`) for every checked code. Each code then gets its own collapsible card with the settings it actually needs — calculation type (SP, Opt, Opt+Freq, Freq, TD-DFT for Gaussian/ORCA, MD for all six, variable-cell relaxation for the four plane-wave codes), functional/method, basis set or cutoff, dispersion, implicit solvent, k-points, pseudopotentials and a free *extra keywords* line — plus an optional override of charge/multiplicities, a **Preview** of configuration 1, and **Edit templates** to override that code's files with custom text (the card then shows *custom template*). Each edit belongs to its file, so changing the calculation type never moves it to another file. QE also offers an optional ph.x input for Γ-point phonons, and CP2K reaches PBE0/B3LYP/HSE06 through ADMM.

Gaussian and ORCA need no cell. The plane-wave codes (QE, VASP, CP2K, Qbox) need one, taken in this order of priority: the card's own **Isolated system: vacuum box** flag (vacuum padding, configuration centred; QE `assume_isolated='mt'`, CP2K Poisson MT, VASP dipole correction), then the card's custom cell (below), then the crystal cell applied in Structure analysis (including one read with **Load cell file**), then the lattice of each trajectory frame (per frame, so NPT trajectories work). Without a cell and with the flag off, Run stays disabled and the status line's **Define cell…** button opens Structure analysis › Cell. A cell arriving with the trajectory (extended XYZ `Lattice=`, or a CIF/cell file given at import) is recognised as soon as the first frame is read, so the cards don't wait for the launcher to fetch frames.

Each plane-wave card that needs a cell shows a **Cell** group, pre-filled from the detected structure cell (a, b, c in Å, α, β, γ in °) with a live preview of the block the code will write. Its *Source* selector switches to **Custom cell for this code**, which edits a, b, c, α, β, γ for that card only — the Structure-analysis crystal cell (and the other cards) are unchanged. A custom cell keeps the orientation of the structure lattice, so it never rotates relative to the atoms (a lattice not in the standard orientation, e.g. the Materials Project hexagonal setting, keeps its a and b directions; the card says so). Loading another trajectory resets every card to the structure cell. The isolated vacuum-box flag hides the Cell group, since the box is built from the configuration's extent instead. Each code writes its native default, with options next to the cell fields:

- **Qbox** — cell and positions always in bohr (Qbox requires it), 8 decimals, shown as the `set cell` command it will emit.
- **QE pw.x** — `CELL_PARAMETERS angstrom` (default), `bohr`, or `alat` (`celldm(1)` = a in bohr); positions Cartesian in the same length unit, or `ATOMIC_POSITIONS crystal` (fractional).
- **CP2K** — `ABC [angstrom]` + `ALPHA_BETA_GAMMA` (default) or `A`/`B`/`C [angstrom]` vectors; positions Cartesian Å or `SCALED .TRUE.` (fractional). `ABC` only applies when the cell is in the standard orientation (**a** along x, **b** in the xy plane); otherwise CP2K falls back to vectors and the preview notes why.
