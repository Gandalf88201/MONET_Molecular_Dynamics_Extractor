...
async function run () {
  ...
  assert.equal(H.session.data.environment.ase, 'test'); assert.equal(H.session.data.environment.python, '3.12.1'); assert.equal(H.session.data.monet_version, fs.readFileSync(path.join(root, 'CITATION.cff'), 'utf8').match(/^version: *(\S+)/m)[1]); checks++
